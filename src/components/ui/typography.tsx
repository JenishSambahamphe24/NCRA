...
const typographyVariants = cva(defClassName, {
  variants: {
    variant: {
      heroTitle:
        "text-4xl font-bold tracking-[-0.01406rem] md:text-5xl md:font-extrabold md:leading-[3.5rem] md:tracking-[-0.075rem]",
      displayText:
        "text-2xl font-bold not-italic leading-8 tracking-[-0.009rem] md:text-4xl md:font-bold md:leading-10 md:tracking-[-0.036rem] lg:text-[2.5rem] lg:font-bold lg:leading-[3rem] lg:tracking-[-0.05rem]",
      ...
      body: "text-sm font-normal not-italic leading-5",
      ...
      headerSmStronger: "text-xl font-semibold leading-7",
      headerXs: "text-base font-medium leading-5",
    },
  },
  defaultVariants: {
    variant: "p",
...
const Typography = React.forwardRef<TypographyElement, TypographyProps>(
  ({ className, variant, asChild = false, loading = false, ...props }, ref) => {
    if (loading)
      return (
        <Skeleton
          className={cn("w-full my-0.5", getSkeletonSize(variant), className)}
        />
      );
